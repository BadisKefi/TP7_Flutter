...
Future getAllClasses() async {
  Response response =
      await http.get(Uri.parse("http://192.168.97.215:8080/class/all"));
  return jsonDecode(response.body);
}

Future deleteClass(int id) {
  return http
      .delete(Uri.parse("http://192.168.97.215:8080/class/delete?id=$id"));
}

Future addClass(Classe classe) async {
  Response response = await http.post(
      Uri.parse("http://192.168.97.215:8080/class/add"),
...
Future updateClasse(Classe classe) async {
  Response response =
      await http.put(Uri.parse("http://192.168.97.215:8080/class/update"),
          headers: {"Content-type": "Application/json"},
          body: jsonEncode(<String, dynamic>{
